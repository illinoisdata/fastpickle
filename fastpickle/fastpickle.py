    ...
    def pardumps(self, obj, pickling_order):
        ...
        def process_bytecodes(parent_list, bytecode, thread, sj, ej):
            ...
            while j < ej:
                ...
                if (thread, j) in defn_replacement:
                ...
                elif b == MEMOIZE and j - 1 >= 0 and bytecode[j - 1] not in int_types: # to do : b = memoize and memoize_mask[t][j] = 1
                    ...
                    if bingetcode in bingets:
                        ...
                        if self.child_bytes[t_num][sp] == EMPTY_LIST or self.child_bytes[t_num][sp] == EMPTY_DICT or self.child_bytes[t_num][sp] == EMPTY_SET:
...
def _pardumps(obj, pickling_order, protocol=None, *, fix_imports=True, buffer_callback=None):
    f = io.BytesIO()
    res = _Pickler(f, protocol, fix_imports=fix_imports, buffer_callback=buffer_callback).pardumps(obj, pickling_order)
    # res = f.getvalue()
    assert isinstance(res, bytes_types)
    return res
